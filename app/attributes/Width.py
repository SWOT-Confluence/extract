...
class Width:
    ...
    ----------
        ...
        width_node: dictionary
            width node-level data organized by reach with nx by 1 (dataframe) values
        width_reach: dictionary
            width reach-level data organized by reach with scalar values
    """
    ...
    def __init__(self, file, topology):
        ...
        df_dict = create_reach_dict(df, self.topology)

        # Create node-level and reach-level dataframes for each reach
        self.width_node = self._create_node_dict(df_dict)
        self.width_reach = create_mean_series(self.width_node)

    def _create_node_dict(self, df_dict):
        ...
        for key, value in df_dict.items():
            ...
            value_tile = np.tile(value.astype("float64").to_numpy(), (1, self.TIME_STEPS))
            # Create a dataframe with repeated width data
            width_df = pd.DataFrame(value_tile, columns = np.arange(0, self.TIME_STEPS))
